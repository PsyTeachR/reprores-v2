...
Load the following data or access the linked CSV files online. Each participant is identified by a unique `user_id`.
...
disgust_scores <- read_csv("https://psyteachr.github.io/psyteachr/data/disgust_scores.csv")
personality_scores <- read_csv("https://psyteachr.github.io/psyteachr/data/personality_scores.csv")
users <- read_csv("https://psyteachr.github.io/psyteachr/data/users.csv")
```
...
Load new user data from [users2](https://psyteachr.github.io/psyteachr/data/users2.csv). Bind them into a single table called `users_all`.
