...
The functions `r hl(`labs())`, `r hl(`xlab())`, and `r hl(`ylab())` are convenient when you just want to change a label name, but the `scale_{aesthetic}_{type}` functions are worth learning because they let you customise many things about any aesthetic property (e.g., `x`, `y`, `colour`, `fill`, `shape`, `linetype`), as long as you choose the correct type (usually `continuous` or `discrete`, but there are also special scale functions for other dta types like dates).
